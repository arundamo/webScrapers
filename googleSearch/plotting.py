...
def plot_circular_outward(data_for_plot, prompt, bg_colour, font_colour, line_colour):
    ...
    for i in range(len(df)):
        ...
        verts = [(center_x, center_y), (center_x + indent * 1.2, center_y), (center_x - indent * 0.2, center_y),
                 (x_pos, y_pos)]
    ...
    if sum(data_for_plot["result"].str.find("*")) != -26:
        ax.text(center_x - 0.3, center_y - 7, "*no relevant results", fontproperties=label_font, fontsize=12,
                ha="right", color=font_colour)
